...
router.post('/', (req, res) => {
  if (!req.body.text) {
    ...
    translate(decodeURIComponent(req.body.text), { to: req.body.to }).then(output => {
      res.status(200).json({ code: 200, message: output.text, lang: { from: output.from.language.iso, to: req.body.to, autocorrect: output.from.text.autoCorrected } });
    }).catch(err => {
